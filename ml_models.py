...
def gp_regression(Xtrain, Ytrain):
    ...
    kernel = train_y_avg**2 * RBF(length_scale=1) + WhiteKernel(
...
def evaluate_ml_models(oect_data):
    ...
    whole_space_scaled = scaler.fit_transform(
        oect_data[feature_columns].values
    )
    whole_space_scaled = pd.DataFrame(
        whole_space_scaled, columns=feature_columns
    )
    ...
    for train_index, test_index in kfold.split(input):
        ...
        results["Linear Regression"]["test_rmse"].append(
            np.sqrt(
                metrics.mean_squared_error(y_test, linear_reg.predict(X_test))
            )
        )
        ...
        if os.path.exists('ml_model_weights/nn_model_weights.h5'):
            ...
            nn_model = (
                build_nn_model()
            )  # Ensure the model architecture is the same
        ...
        results["Neural Net"]["train_rmse"].append(
            np.sqrt(
                metrics.mean_squared_error(y_train, nn_model.predict(X_train))
            )
        )
        results["Neural Net"]["test_rmse"].append(
            np.sqrt(
                metrics.mean_squared_error(y_test, nn_model.predict(X_test))
            )
        )

        # Random Forest
        rf_reg = RandomForestRegressor(n_estimators=2000, random_state=0).fit(
            X_train, y_train
        )
        ...
        # AdaBoost Regressor
        adaboost = AdaBoostRegressor(random_state=0, n_estimators=1000).fit(
            X_train, y_train
        )
        results["AdaBoost"]["train_rmse"].append(
            np.sqrt(
                metrics.mean_squared_error(y_train, adaboost.predict(X_train))
            )
        )
        results["AdaBoost"]["test_rmse"].append(
            np.sqrt(
                metrics.mean_squared_error(y_test, adaboost.predict(X_test))
            )
        )

    models = list(results.keys())
    for model in models:
        results[model]["train_rmse_average"] = np.average(
            results[model]["train_rmse"]
        )
        results[model]["test_rmse_average"] = np.average(
            results[model]["test_rmse"]
        )
...
def shapley_analysis_plotly(oect_data):
    ...
    mdl = RandomForestRegressor(
        n_estimators=1000, max_depth=20, random_state=888
    )  # max_depth=12, random_state=888)
    ...
    combined_df['Feature'] = pd.Categorical(
        combined_df['Feature'], categories=shap_range.index, ordered=True
    )
    ...
    fig = px.scatter(
        ...
        color='Feature Value',
        labels={
            'SHAP Value': 'SHAP Value (Impact on Model Output)',
            'Feature': 'Feature (Ranked by Extremes)',
        },
...
def precompute_results(start_idx, end_idx):
    ...
    with open(
        f"ml_model_weights/results_{start_idx}_{end_idx}.json", 'w'
    ) as f:
        json.dump(results, f)
