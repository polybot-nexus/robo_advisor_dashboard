...
for i, row in remaining_rows.iterrows():
    new_dataset = pd.concat([new_dataset, pd.DataFrame([row])], ignore_index=True)
    print(f"Row {i} added to the new dataset.")
    
    # Save the updated new_dataset to CSV every time a row is added
    new_dataset.to_csv('datasets/oect_summary_posted_rf__plus_ml_combined.csv', index=False)
