...
def create_plotly_stock_market_plot(oect_data):
    ...
    fig = make_subplots(
        ...
        shared_xaxes=True,
        subplot_titles=(
            "Transconductance and Moving Averages",
            "MACD Histogram",
        ),
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index[21:],
            y=df['transconductance'][21:],
            mode='markers',
            marker=dict(
                color='white', size=10, line=dict(width=2, color='black')
            ),
            name='GPR + RF',
        ),
        row=1,
    ...
    for i in range(21, len(df['transconductance'] + 1), bin_width):
        ...
        if len(bin_data) == bin_width:
            box_color = (
                '#D6B5B3'
                if bin_data.iloc[0] < bin_data.iloc[-1]
                else '#B2C8D8'
            )
            stats = {
    ...
    try:
        ...
        fig.add_trace(
            go.Bar(
                x=df.index,
                y=df['MACDh_10_20_8'],
                marker=dict(color=colors, line=dict(color='black')),
                name='Histogram',
            ),
            row=2,
            col=1,
    ...
    fig.update_xaxes(
        showline=True, linewidth=2, linecolor='black', row=2, col=1
    )
    fig.update_yaxes(
        showline=True, linewidth=2, linecolor='black', row=1, col=1
    )
    fig.update_yaxes(
        showline=True, linewidth=2, linecolor='black', row=2, col=1
    )
